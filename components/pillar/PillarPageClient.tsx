...
import {
  ...
  ArrowRight, ArrowLeft, ListChecks, ChevronDown,
  BookOpen, Target, TrendingUp, Clock,
} from "lucide-react";
import type { PillarData, CriterionData } from "./pillarData";

// ─────────────────────────────────────────────────────────────────────────────
// ICON RESOLVER — maps string names to components (avoids Server→Client error)
// ─────────────────────────────────────────────────────────────────────────────

const ICON_MAP: Record<string, React.ElementType> = { Brain, Gauge, Shield, Sparkles };

const BRAND    = "#004DFD";
const BRAND_LT = "#5B8CFF";
const ANCHOR_COLORS: Record<number, string> = { 0: "#C94040", 6: "#E07840", 12: "#8B9BB2", 18: "#4A9960", 25: "#004DFD" };

// ─────────────────────────────────────────────────────────────────────────────
// SCORING ACCORDION — shows anchors for each criterion
// ─────────────────────────────────────────────────────────────────────────────

function ScoringAccordion({ crit }: { crit: CriterionData }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50/50 dark:border-white/5 dark:bg-white/[0.02]">
      <button onClick={() => setOpen(!open)}
        className="flex w-full items-center justify-between px-4 py-3 text-left text-xs font-bold uppercase tracking-wider transition-colors hover:bg-slate-100/50 dark:hover:bg-white/[0.03]"
        style={{ color: BRAND }}>
        <span className="flex items-center gap-2">
          <BookOpen className="h-3.5 w-3.5" />
          Scoring Scale for {crit.id}
        </span>
        ...
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="border-t border-slate-100 dark:border-white/5">
              {crit.anchors.map((a, i) => (
                <div key={a.score} className="flex gap-3 px-4 py-2.5"
                  style={{ borderBottom: i < 4 ? "1px solid rgba(0,0,0,0.04)" : "none" }}>
                  <span className="w-7 shrink-0 text-center text-sm font-black tabular-nums" style={{ color: ANCHOR_COLORS[a.score] }}>
                    {a.score}
                  </span>
                  <span className="text-xs leading-5 text-slate-600 dark:text-slate-400">{a.text}</span>
                </div>
              ))}
            </div>
          </motion.div>
...
// ─────────────────────────────────────────────────────────────────────────────
// MAIN — UNIFIED PILLAR PAGE
// ─────────────────────────────────────────────────────────────────────────────

export default function PillarPageClient({ pillar }: { pillar: PillarData }) {
  const Icon = ICON_MAP[pillar.iconName] || Brain;
  ...
  return (
    <main className="relative bg-white dark:bg-[#050B1F]">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-50 to-white px-4 py-24 dark:from-[#0A0F1E] dark:to-[#050B1F] sm:px-6 lg:px-8 lg:py-32">
        ...
        <div className="relative mx-auto max-w-5xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
            ...
            style={{ borderColor: `${BRAND}40`, background: `${BRAND}0F`, color: BRAND }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
              <div className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-300">
                ...
                Scored 0–100 points (4 domains × 25)
              </div>
              <div className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-300">
      ...
      {/* ═══ SECTION 2: WHY IT MATTERS (with image) ═══ */}
      <section ref={section2Ref} className="relative px-4 py-20 bg-white dark:bg-[#070D1B] sm:px-6 lg:px-8 lg:py-28">
        <div className="mx-auto max-w-7xl">
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-16">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={isSection2InView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }}>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={isSection2InView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }}
              className="relative">
              <div className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-100 shadow-2xl dark:border-white/10 dark:bg-white/[0.03]">
                {/* Image slot — place pillar-specific image at public/pillars/{slug}.jpg */}
               {/* Dynamic Image component for Section 2 */}
<div className="relative aspect-[4/3] w-full overflow-hidden bg-slate-100 dark:bg-slate-900">
  <Image 
    src={`/${pillar.seo.slug}.png`} 
    alt={`${pillar.name} - Governing Law`} 
    fill
  ...
  />
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-16 text-center">
            ...
            <p className="mx-auto max-w-2xl text-lg text-slate-600 dark:text-slate-400">
              ...
              Every factor has exactly one home — no overlap, no double-counting.
              Click any domain to see its full scoring scale.
            </p>
          </div>
            ...
            {pillar.criteria.map((crit, i) => (
              <motion.div key={crit.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="mb-5 flex items-start gap-4">
                  <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl text-xl font-black text-white shadow-lg" style={{ background: BRAND }}>
                    {crit.id}
                  </div>
                  ...
                  <div className="hidden shrink-0 items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-500 sm:flex dark:border-white/10 dark:bg-white/[0.03]">
                    <Gauge className="h-3 w-3" style={{ color: BRAND }} />
                    0–25 pts
                  </div>
                </div>
                ...
                <div className="mb-5">
                  <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-500">
                    <span className="flex items-center gap-1.5"><ListChecks className="h-3.5 w-3.5" style={{ color: BRAND }} /> What belongs in this domain</span>
                  </p>
      ...
      <section ref={caseRef} className="relative px-4 py-20 bg-slate-50 dark:bg-[#0A0F1E] sm:px-6 lg:px-8 lg:py-28">
        <div className="mx-auto max-w-6xl">
         ...
         {/* Dynamic Image component for Section 5 */}
<motion.div initial={{ opacity: 0, y: 30 }} animate={isCaseInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}
  className="mb-12 relative overflow-hidden rounded-3xl border border-slate-200 bg-slate-100 shadow-2xl dark:border-white/10 dark:bg-white/[0.03]">
  <div className="relative aspect-[16/9] w-full">
    <Image 
      src={`/${pillar.seo.slug}-comparison.png`} 
      alt={`${pillar.name} - Pass vs Fail Architecture`} 
      fill
      className="object-cover"
    />
  </div>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={isCaseInView ? { opacity: 1, x: 0 } : {}}
              ...
              <p className="mb-4 text-sm font-bold uppercase tracking-wider text-red-600 dark:text-red-500">
                Automatic Failure — {pillar.governingLaw} Triggered
              </p>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={isCaseInView ? { opacity: 1, x: 0 } : {}}
              ...
              <p className="mb-4 text-sm font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-500">LIONXE™ Pass</p>
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-12 text-center">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-400">
              The five-level scale applied to each of the four domains above
            </p>
          </div>

          <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-white/10">
            {[
              { score: 0,  label: "Absent", color: "#C94040", desc: "Affirmative evidence of failure — not simply absent evidence." },
              { score: 6,  label: "Minimal", color: "#E07840", desc: "Available evidence does not confirm the standard. Also the no-evidence default." },
              { score: 12, label: "Partial", color: "#8B9BB2", desc: "Present in some areas, absent in others — roughly half meets the standard." },
              { score: 18, label: "Substantial", color: "#4A9960", desc: "Present across most of the entity with minor identified exceptions." },
              { score: 25, label: "Complete", color: BRAND, desc: "Consistently present everywhere reviewed. All factors in the domain are met." },
            ].map((s, i) => (
              <div key={s.score} className="flex items-center gap-5 px-6 py-4"
                style={{ borderBottom: i < 4 ? "1px solid rgba(0,0,0,0.06)" : "none", background: i % 2 === 0 ? "transparent" : "rgba(0,0,0,0.01)" }}>
                <span className="w-9 shrink-0 text-center text-xl font-black tabular-nums" style={{ color: s.color }}>{s.score}</span>
                <div>
                  <span className="text-sm font-bold text-slate-900 dark:text-white">{s.label}</span>
                  <span className="ml-2 text-sm text-slate-500 dark:text-slate-400">— {s.desc}</span>
                </div>
              </div>
            ))}
          </div>

          <div className="mt-6 rounded-xl border border-orange-200 bg-orange-50 p-5 dark:border-orange-500/20 dark:bg-orange-500/[0.06]">
            <div className="flex items-start gap-3">
              <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-orange-600 dark:text-orange-500" />
              <p className="text-sm leading-6 text-orange-900 dark:text-orange-200">
                <strong className="font-semibold">Blocking rule:</strong> If this pillar totals below 15/100, or if any criterion scores 0, the issue is flagged as blocking and categorically excludes certification until resolved — regardless of the grand total.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="relative px-4 py-20 bg-slate-900 dark:bg-[#0A0F1E] sm:px-6 lg:px-8 lg:py-32">
        <div className="mx-auto max-w-4xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <Icon className="mx-auto mb-6 h-12 w-12 text-white" />
            <h2 className="mb-6 text-4xl font-extrabold text-white sm:text-5xl">The Self-Auditing Protocol</h2>
            <p className="mb-12 text-lg text-slate-400">Test this gate yourself — before submitting for formal evaluation</p>
          </motion.div>
            ...
            {pillar.selfAuditQuestions.map((q, i) => (
              <div key={i} className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-md">
                <div className="flex gap-4">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-lg font-bold text-white" style={{ background: BRAND }}>
                    {i + 1}
                  </div>
                  <p className="leading-7 text-slate-300">{q}</p>
                </div>
              </div>
            ))}
          </motion.div>
      ...
      <section className="relative px-4 py-16 bg-white dark:bg-[#050B1F] sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <a href="/rubric" className="inline-flex items-center gap-2 rounded-xl px-6 py-3 text-sm font-bold text-white transition-all hover:-translate-y-0.5"
